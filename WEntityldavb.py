...
def parse_doc_list(docs, vocab):
    ...
    wordids = list()
    wordcts = list()
    topics=[topic.replace('\n','') for topic in file('./Entity/Topics.txt').readlines()]
    for d in range(0, D): 
        mywords,_,mywordcts=zip(*docs[d])
        
        tinwords=[mywords.index(w) for w in mywords if w in topics]
#        
        tempwordcts=list()
        for i,wcts in enumerate(mywordcts):
            tempwordcts.append(wcts)
            if i in tinwords: tempwordcts[i]= wcts*1
         
        mywordcts=tempwordcts   
...
#                ddict[wordtoken] += 1
        mywordids=list()
        for word in mywords:
            mywordids.append(vocab[word])
        
        wordids.append(mywordids)
        wordcts.append(mywordcts)
...
class WLDA:
    ...
    def __init__(self, vocab, K, D, alpha, eta, tau0, kappa, _lambda):
        ...
        vocab=[l for l in set(vocab.values())]
